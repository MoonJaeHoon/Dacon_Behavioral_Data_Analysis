...
* 이것은 한가지 아쉬운 점인데, LightGBM 적합시에 gbdt모드를 설정하여 모든 코드구현이 이루어졌지만, 일반적으로 성능이 더 높을 수 있는 dart모드의 LightGBM을 시간이 너무 많이 소모되어 시도하지 못한 것이 아쉬움.
* 오토인코더를 활용한 차원축소 후 예측 모델링 (오토인코더를 통한 차원축소 시도에 있어서 loss가 큰 문제, 이에 따라 예측력 역시 좋지 못했음.)
* 
