...
* https://dacon.io/myprofile/226309/overview/
...
### train.csv (67091776 rows, 7 columns) / test.csv (28714849 rows, 6 columns)
...
* LGBM과 같은 boosting_rounds 횟수를 튜닝해주어야 하는 모델에 있어서는 Early_Stopping method가 중요할 것이다.(이를 Optuna 라이브러리는 지원하고 있다.)
* AutoML을 쓸 때 주의해야 한다고 생각한 부분은 최대한 narrow한 range에서 파라미터 튜닝을 따로 수차례 하는 것이 좋다는 점이다.
...
* 위와 같은 방법을 활용한, 단일모델로서는 Ch5.2의 모델이 가장 높은 점수를 가져다 주었지만
* 서로 다르게 변수를 선택하여 적합시킨 LightGBM 모델 2개 (Ch5.1 & Ch5.2) 예측값들을 평균내어 앙상블한 것이 더 높은 점수를 가져다 주었다. (AUC=0.754, 리더보드 6위)

### 위와 같은 결론을 얻기까지 시도한 다양한 방법들 정리
* Camera 좌표들의 Plot 그림 자체를 변수로 이용하는 딥러닝 모델 (CNN, DNN 등 시도하였지만 성능이 나오지 않아 사용하지 못 함.)
* Camera 좌표들을 활용하기 위해 스타팅 위치좌표, Player0와 Player1 스타팅 위치좌표 사이의 거리, 스타팅 위치좌표로부터 움직인 좌표들의 SSW(게임내 좌표의변동) 등을 추가해보았지만 모델 성능에 도움이 되지 않음.
* 시간의 구간을 1분, 3분, 4분, 6분 등 다양하게 시도해보았지만 이들 중 가장 예측력이 높았던, 2분씩 나누는 구간별 변수를 생성함.
* 모든 변수들이 이벤트가 발생한 횟수로 생성되기에 차이변수인 diff변수를 생성할 때 변수 그자체의 차이보다는 Root 변환 후 diff변수를 생성하였지만 별 도움이 되지 못함.
* 오토인코더를 활용한 차원축소 후 예측 모델링 (오토인코더를 통한 차원축소 시도에 있어서 loss가 크다는 문제, 이에 따라 예측력 역시 좋지 못했음.)
* 이것은 한가지 아쉬운 점인데, LightGBM 적합시에 gbdt모드를 설정하여 모든 코드구현이 이루어졌지만, 일반적으로 성능을 더 높게 만든다고 알려진 dart모드의 LightGBM은 시간이 너무 많이 소모되어 시도하지 못한 것이 아쉬움.
